...
protected:
    void SetUp() override {
        ...
        ASSERT_TRUE(file_exists(echo_worker_)) << "echo-worker.js not found at: " << echo_worker_
    ...
    std::vector<std::string> send_and_collect(
        Bus& bus, const std::vector<std::string>& messages, int expected_responses,
        std::chrono::seconds timeout = std::chrono::seconds(10)) {
        std::vector<std::string> responses;

        bus.on_message([&](std::string_view msg) { responses.emplace_back(msg); });
...
TEST_F(Conformance, SingleRequestResponse) {
    ...
    std::string request =
        R"({"jsonrpc":"2.0","id":"conf-single-1","method":"echo","params":{"message":"hello from cpp sdk"},"sessionId":"session-conf-1","pool":"echo-worker"})";
...
TEST_F(Conformance, MultipleConcurrentMessages) {
    ...
    for (int i = 1; i <= NUM_CLIENTS; ++i) {
        requests.push_back(R"({"jsonrpc":"2.0","id":"concurrent-)" + std::to_string(i) +
...
TEST_F(Conformance, SessionAffinityRouting) {
    ...
    for (auto& session : sessions) {
        requests.push_back(R"({"jsonrpc":"2.0","id":"route-)" + session +
                           R"(","method":"echo","params":{"session":")" + session +
                           R"("},"sessionId":"session-)" + session + R"(","pool":"echo-worker"})");
    }
    ...
    for (auto& session : sessions) {
        requests2.push_back(R"({"jsonrpc":"2.0","id":"route-)" + session +
    ...
    for (auto& resp : responses2) {
        ...
        EXPECT_FALSE(session_id.empty())
            << "Session affinity: sessionId must be preserved on second request";
    }
...
TEST_F(Conformance, RequestResponseCorrelation) {
    ...
    for (int i = 1; i <= NUM; ++i) {
        requests.push_back(R"({"jsonrpc":"2.0","id":"corr-)" + std::to_string(i) +
...
TEST_F(Conformance, GracefulShutdown) {
    ...
    std::string request =
        R"({"jsonrpc":"2.0","id":"shutdown-1","method":"echo","params":{},"pool":"echo-worker"})";
    auto responses = send_and_collect(bus, {request}, 1);
...
TEST_F(Conformance, StatisticsAccuracy) {
    ...
    std::vector<std::string> requests;
    for (int i = 1; i <= NUM; ++i) {
        requests.push_back(R"({"jsonrpc":"2.0","id":"stats-)" + std::to_string(i) +
...
TEST_F(Conformance, MultiWorkerDistribution) {
    ...
    for (int i = 1; i <= NUM_MESSAGES; ++i) {
        requests.push_back(R"({"jsonrpc":"2.0","id":"dist-)" + std::to_string(i) +
                           R"(","method":"echo","params":{"n":)" + std::to_string(i) +
                           R"(},"pool":"echo-worker"})");
    }
...
TEST_F(Conformance, LargeMessageHandling) {
    ...
    EXPECT_GT(resp.size(), 60000u) << "Response must contain the echoed large payload";
...
TEST_F(Conformance, NotificationNoResponse) {
    ...
    bus.on_message([&](std::string_view msg) { received.emplace_back(msg); });

    // Send a notification (no id field) - per JSON-RPC 2.0, no response expected
    std::string notification =
        R"({"jsonrpc":"2.0","method":"notify","params":{"event":"test"},"pool":"echo-worker"})";
    ...
    std::string request =
        R"({"jsonrpc":"2.0","id":"after-notify","method":"echo","params":{},"pool":"echo-worker"})";
...
TEST_F(Conformance, InlineJsonConfig) {
    ...
    auto bus = BusBuilder().config_json(config).log_level(2).build();
    ...
    std::string request =
        R"({"jsonrpc":"2.0","id":"inline-conf-1","method":"echo","params":{"source":"inline"},"pool":"echo-worker"})";
    auto responses = send_and_collect(bus, {request}, 1);
