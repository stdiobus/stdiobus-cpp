...
TEST(E2E, MultipleMessages) {
    ...
    for (int i = 0; i < NUM_MESSAGES; ++i) {
        std::string msg = R"({"jsonrpc":"2.0","method":"test","id":")" + 
                          std::to_string(i + 1) + R"("})";
        err = bus.send(msg);
...
TEST(E2E, StatsTracking) {
    ...
    // Wait for responses
    auto deadline = std::chrono::steady_clock::now() + std::chrono::seconds(3);
    while (static_cast<int>(received.size()) < NUM && 
           std::chrono::steady_clock::now() < deadline) {
        bus.step(std::chrono::milliseconds(50));
    }
...
TEST(E2E, LargeMessage) {
    ...
    std::string msg = R"({"jsonrpc":"2.0","method":"large","params":{"data":")" + 
                      payload + R"("},"id":"large-1"})";
