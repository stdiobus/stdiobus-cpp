...
class AsyncBus {
public:
    explicit AsyncBus(std::string_view config_path) : bus_(config_path), next_id_(1) {
        setup_message_handler();
    }

    explicit AsyncBus(Options options) : bus_(std::move(options)), next_id_(1) {
        setup_message_handler();
    }
    ...
    int pump(Duration timeout = Duration::zero()) { return bus_.step(timeout); }

    /// Send async request, returns future
    std::future<AsyncResult> request_async(
        std::string message, std::chrono::milliseconds timeout = std::chrono::seconds(30)) {
...

        {
            ...
            pending_[id] =
                PendingRequest{std::move(promise), std::chrono::steady_clock::now() + timeout};
        }
    ...
    [[nodiscard]] Error notify(std::string_view message) { return bus_.send(message); }
        ...
        for (auto it = pending_.begin(); it != pending_.end();) {
            if (now >= it->second.deadline) {
                it->second.promise->set_value(
                    AsyncResult{Error(ErrorCode::Timeout, "Request timed out"), ""});
...
private:
    ...
    void setup_message_handler() {
        bus_.on_message([this](std::string_view msg) { handle_response(msg); });
    }
