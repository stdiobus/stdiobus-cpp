...
public:
    ...
    int start() noexcept { return stdio_bus_start(handle_); }

    int step(int timeout_ms) noexcept { return stdio_bus_step(handle_, timeout_ms); }

    int stop(int timeout_sec) noexcept { return stdio_bus_stop(handle_, timeout_sec); }

    int ingest(const char* msg, size_t len) noexcept { return stdio_bus_ingest(handle_, msg, len); }

    stdio_bus_state_t get_state() const noexcept { return stdio_bus_get_state(handle_); }

    int worker_count() const noexcept { return stdio_bus_worker_count(handle_); }

    int session_count() const noexcept { return stdio_bus_session_count(handle_); }

    int pending_count() const noexcept { return stdio_bus_pending_count(handle_); }

    int client_count() const noexcept { return stdio_bus_client_count(handle_); }

    int get_poll_fd() const noexcept { return stdio_bus_get_poll_fd(handle_); }

    void get_stats(stdio_bus_stats_t* stats) const noexcept { stdio_bus_get_stats(handle_, stats); }
...
inline Stats to_stats(const stdio_bus_stats_t& s) noexcept {
    return Stats{s.messages_in,     s.messages_out,   s.bytes_in,        s.bytes_out,
                 s.worker_restarts, s.routing_errors, s.client_connects, s.client_disconnects};
}
